...
public class EventsViewPage extends FragmentActivity implements AdvancedWebView.Listener{


    private EventsModel pageItem;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.head_drawer_title)).setText(Concurrent.getLangSubWords("events", "Events"));
        ...
        if(pageID != 0) loadEventsItem(pageID);
        else {
    ...
    public void loadEventsItem(Integer id){
        if(id == 0){
            changePageView(pageLayer.ErrorLoading);
            return;
        }
        ...
        if (TOKEN != null) {
            ...
            Request.Builder requestBuilder = new Request.Builder()
                    .url(App.getAppBaseUrl() + Constants.TASK_EVENTS_ITEM + "/" + id);
                                ...
                                try {
                                    if (responseObj.isSuccessful()) {
                                        ...
                                        if(ValuesHolder != null ){
                                            ...
                                            pageItem = new EventsModel(Concurrent.tagsIntValidator(ValuesHolder, "id"),
                                                    Concurrent.tagsStringValidator(ValuesHolder, "eventTitle"),
                                                    Concurrent.tagsStringValidator(ValuesHolder, "eventDescription"),
                                                    Concurrent.tagsStringValidator(ValuesHolder, "eventDate"),
                                                    Concurrent.tagsStringValidator(ValuesHolder, "eventFor"),
                                                    Concurrent.tagsStringValidator(ValuesHolder, "enentPlace"));

                                            ((ParentStyledTextView) findViewById(R.id.header_title)).setNotNullText(pageItem.title);
                                            ((ParentStyledTextView) findViewById(R.id.footer_date_data)).setNotNullText(pageItem.date);
                                            ((ParentStyledTextView) findViewById(R.id.footer_place_data)).setNotNullText(pageItem.place);
