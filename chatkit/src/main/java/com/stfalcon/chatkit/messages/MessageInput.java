...
public class MessageInput extends RelativeLayout
    ...
    private AVLoadingIndicatorView messageSendButtonProgress;

    private sendButtonStatus buttonStatus = sendButtonStatus.ENABLED;
    ...
    public void onTextChanged(CharSequence s, int start, int count, int after) {
        input = s;
        if(buttonStatus.equals(sendButtonStatus.ENABLED)) messageSendButton.setEnabled(input.length() > 0);
    }
    ...
    private void init(Context context) {
        ...
        messageSendButtonProgress = (AVLoadingIndicatorView) findViewById(R.id.messageSendButtonProgress);
    ...
    /**
     * Change Send Button Status
     */

    public void changeButtonStatus(sendButtonStatus status){

        this.buttonStatus = status;

        if(status.equals(sendButtonStatus.ENABLED)){

            this.messageSendButton.setVisibility(VISIBLE);
            this.messageSendButton.setEnabled(true);
            this.messageSendButtonProgress.setVisibility(GONE);

        }else if(status.equals(sendButtonStatus.DISABLED)){

            this.messageSendButton.setVisibility(VISIBLE);
            this.messageSendButton.setEnabled(false);
            this.messageSendButtonProgress.setVisibility(GONE);

        }else if(status.equals(sendButtonStatus.LOADING)){

            this.messageSendButton.setVisibility(GONE);
            this.messageSendButton.setEnabled(false);
            this.messageSendButtonProgress.setVisibility(VISIBLE);
